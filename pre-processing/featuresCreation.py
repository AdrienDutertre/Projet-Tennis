...
def getNbGamesPlayed(days, date, playerName, dataframe) :

	# Extraction des matchs dans un intervalle de 15 jours 
	dataframe.loc[:, 'Date'] = pd.Series( pd.to_datetime( dataframe.loc[:, 'Date'].values, dayfirst=True ) ) 
	potentialMatchs = dataframe.set_index('Date')
	antDate = date - datetime.timedelta(days=15)
	date = date - datetime.timedelta(days=1)
	print antDate
	print date
	#potentialMatchs = potentialMatchs.sort_index().ix[ antDate:date ]
	potentialMatchs = potentialMatchs.ix[ antDate:date ]

	# Les matchs du joueur en tant que J0 et J1
	j0Games = potentialMatchs[ potentialMatchs['J0'] == playerName ]
	j1Games = potentialMatchs[ potentialMatchs['J1'] == playerName ]
	playerGames = pd.concat( [j0Games, j1Games] )

	# Maintenant on ne retient que les les colonnes jeux
	playerGames = playerGames[ ['J0Set1', 'J1Set1', 'J0Set2', 'J1Set2', 
		  						'J0Set3', 'J1Set3', 'J0Set4', 'J1Set4',
		  						'J0Set5', 'J1Set5'
		  					 ] ]

	# Et on retourne le nombre de jeux joués au total par le joueur dans l'intervalle 
	return playerGames.sum().sum()
